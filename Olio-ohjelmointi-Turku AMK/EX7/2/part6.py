...
def roll_two_dices():
    return sum([Dice().roll() for _ in range(2)])

def select_mammal_based_on_roll(roll_sum):
    # Example of mapping dice roll sum to mammal selection logic
    if roll_sum <= 6:
        return Mammal(1, "Cat", "Whiskers", "Small", 4)
    elif roll_sum <= 9:
        return Mammal(2, "Dog", "Rex", "Medium", 25)
    elif roll_sum <= 12:
        return Mammal(3, "Elephant", "Dumbo", "Large", 6000)
    else:
        return Mammal(4, "Lion", "Simba", "Large", 200)
...
# Roll dices for each player to determine their pet mammal
for player in players.values():
    roll_sum = roll_two_dices()
    print(f"{player.name} rolls the dice: Sum = {roll_sum}")
    pet = select_mammal_based_on_roll(roll_sum)
    player.set_pet(pet)
